...
sealed class State<T> {
    ...
    data class ResponseError<T>(val code: Int, val message: String, val errorBody: Any? = null) :
        State<T>() //api returned error (403, 404, etc.)

    data class ExceptionError<T>(val errorMessage: String, val throwable: Throwable? = null) :
        State<T>() //local error (no internet, parsing error, etc.)
}
